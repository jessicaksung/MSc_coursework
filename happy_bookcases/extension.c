...
#include <string.h>
#include <ctype.h>
#include "simple_screen/neillsimplescreen.h"
...
/* print book shelf in black and white */
void printBookcase(bookcase b[ARRSIZE], int index);
/* print book shelf in color */
void printColorBookcase(bookcase b[ARRSIZE], int index);
...
void initBookcases(bookcase b[ARRSIZE], char initialShelf[MAXHEIGHT][MAXWIDTH],
                                        int h, int w);
/* checks for duplicate bookshelfs */
bool isDuplicate(bookcase b[ARRSIZE], int index);
/* search optimized to remove duplicate bookshelf*/
void addBookcaseOptimized(bookcase b[ARRSIZE], int h, bool printFlag);
...
int main (int argc, char** argv) {
  ...
  addBookcaseOptimized(bookcases, height, printFlag);
  return 0;
}
...
void printColorBookcase(bookcase b[ARRSIZE], int index) {
   ...
   for (j = 0; j < b[index].h; j++) {
     for (i = 0; i < b[index].w; i++) {
       color = toupper(b[index].shelf[j][i]);
       switch(color) {
         case BLACK:
           neillfgcol(black);
           break;
...
bool isDuplicate(bookcase b[ARRSIZE], int index) {
  ...
  for (k = 0; k < index; k++) {
    ...
    for (j = 0; j < b[index].h; j++) {
      for (i = 0; i < b[index].w; i++) {
        if (b[k].shelf[j][i] != b[index].shelf[j][i]) {
          flag = false;
        }
      }
    }
        ...
        while(k != j && !happyFlag) {
          if (hasSpace(b, headIndex, k)) {
            ...
            if (currentIndex <= (ARRSIZE - 1)) {
              ...
              /* if it's a duplicate, remove from bookcases array */
              if (isDuplicate(b, currentIndex)) {
                currentIndex--;
              } else if (isHappy(b, currentIndex)) {
                happyFlag = true;
...
/* old search technique with duplicates included */
void addBookcase(bookcase b[ARRSIZE], int h, bool printFlag) {
  int headIndex = 0, currentIndex = 0, j, k, happyIndex = 0;
  bool happyFlag = false;
  char book;
...
void test(void) {
  ...
  addBookcaseOptimized(b1, 3, true);
  assert(countSolution(b1, 20) == 4);
  ...
  addBookcaseOptimized(b1, 2, true);
  ...
  addBookcaseOptimized(b1, 5, true);
  ...
  addBookcaseOptimized(b1, 2, true);
  /* test that there's duplicate bookcases */
  printf("test for search with duplicates\n");
  shelf1[0][0] = YELLOW;
  shelf1[0][1] = EMPTY;
  shelf1[0][2] = EMPTY;
  shelf1[1][0] = BLUE;
  ...
  initBookcases(b1, shelf1, 3, 3);
  addBookcase(b1, 3, true);
  assert(isDuplicate(b1, 10));
  /* test optimized search with duplicates removed */
  printf("test for search with duplicates removed\n");
  shelf1[0][0] = YELLOW;
  shelf1[0][1] = EMPTY;
  shelf1[0][2] = EMPTY;
  shelf1[1][0] = BLUE;
